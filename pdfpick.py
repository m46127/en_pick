...
def pdfpick_page():
    ...
    if uploaded_file_pdf is not None and uploaded_file_excel is not None:
        ...
        inventory_df['在庫コード'] = inventory_df['在庫コード'].astype(str).str.strip().str.upper()

        merged_df = pd.merge(inventory_df, df, left_on='在庫コード', right_on='Product Code', how='left')
        merged_df = merged_df[['在庫コード', 'Quantity']]
